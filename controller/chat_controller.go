...
func (c *Controller) Chat(w http.ResponseWriter, r *http.Request, claims TokenClaims, _ UserRecord) {
	...
	_, _ = c.db.Exec(`INSERT INTO chat_messages (conversation_id,user_id,role,content,metadata) VALUES ($1,$2,'user',$3,'{}'::jsonb),($1,$2,'assistant',$4,'{}'::jsonb)`,
		convID, claims.UserID, body.Message, answer)
	_, _ = c.db.Exec(`UPDATE chat_conversations SET message_count=message_count+2,last_message_preview=$2,last_message_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$1`,
		convID, body.Message)
...
func (c *Controller) ChatSessions(w http.ResponseWriter, _ *http.Request, claims TokenClaims, _ UserRecord) {
	...
	for rows.Next() {
		...
		_ = rows.Scan(&id, &status, &cnt, &preview, &lm, &created, &updated)
